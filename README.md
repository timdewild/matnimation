...
\end{align}
```
over the time interval $t\in [0,2\pi]$. Go to the [code](\examples\oscillating_particle/oscillating_particle.py). 
...
\end{equation}
```
where $\omega$ is the angular frequency and $k$ is the wavenumber. Go to the [code](\examples\travelling_waves/travelling_waves.py). 
